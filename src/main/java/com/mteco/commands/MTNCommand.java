...
import com.mteco.inventory.impl.nation.AdminNationGUI;
import com.mteco.inventory.impl.nation.CustomRoleDashboardGUI;
import com.mteco.inventory.impl.nation.CustomRolesGUI;
import com.mteco.inventory.impl.nation.NationCharacterSelectGUI;
import com.mteco.inventory.impl.nation.NationCitizenGUI;
import com.mteco.inventory.impl.nation.NationColorSelectGUI;
import com.mteco.inventory.impl.nation.NationManagementGUI;
import com.mteco.inventory.impl.nation.NationElitesGUI;
import com.mteco.inventory.impl.nation.NationSecurityGUI;
import com.mteco.inventory.impl.nation.VicePresidentManagementGUI;
import com.mteco.inventory.impl.nation.TreasurerGUI;
import com.mteco.inventory.impl.nation.NationPayGUI;
import com.mteco.inventory.impl.nation.CouncilVotesGUI;
...
public class MTNCommand implements CommandExecutor, TabCompleter {
    private final MTeco plugin;

    public MTNCommand(MTeco plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!plugin.isMTNationsEnabled()) {
            player.sendMessage("§cMTNations is not enabled on this server.");
            return true;
        }

        // /mtn ns — National Security Head
        if (args.length >= 1 && args[0].equalsIgnoreCase("ns")) {
            ...
            if (secNation.isCustomGovernment()) {
                player.sendMessage("§cUse §e/mtn §cto open your nation dashboard.");
                return true;
            }
        ...
        // /mtn pay — President or Treasury permission
        ...
        if (args.length >= 1 && args[0].equalsIgnoreCase("borders")) {
            ...
            if (enabled) {
                player.sendMessage("§aRegion borders enabled. Run §6/mtn borders§a again to turn off.");
            } else {
        ...
        if (args.length >= 1 && args[0].equalsIgnoreCase("pborders")) {
            ...
            if (enabled) {
                player.sendMessage("§aProperty borders enabled. Run §6/mtn pborders§a again to turn off.");
            } else {
        ...
        // /mtn admin [define nation]
        if (args.length >= 1 && args[0].equalsIgnoreCase("admin")) {
            if (!player.hasPermission("MTNations.admin")) {
                return true;
            }
            if (args.length >= 3 && args[1].equalsIgnoreCase("define") && args[2].equalsIgnoreCase("nation")) {
        ...
        // /mtn pcr <regionName> <price>
        if (args.length >= 3 && args[0].equalsIgnoreCase("pcr")) {
            handlePCR(player, args);
            return true;
        }

        // /mtn — auto-detect role and open appropriate GUI
    ...
    private void handleJoinRequest(Player player, String[] args) {
        ...
        if (president != null) {
            ...
            president.sendMessage("§e" + reqName + " §7has requested to join your nation. Use §e/mtn requests§7 to view.");
        }
    }
    ...
    private void handleViewRequests(Player player) {
        ...
        player.sendMessage("§7Use §e/mtn accept <player> §7or §e/mtn deny <player>");
    ...
    private void handlePCR(Player player, String[] args) {
        ...
        if (nation == null && !player.hasPermission("MTNations.admin")) {
    ...
    private void handleClaimBiome(Player player) {
        ...
        if (regionId == null) {
            player.sendMessage("§cYou have no targeted region. Use the Region Management GUI (/mtr) to target a region first.");
            return;
        }
    ...
    private void handleEnforcers(Player player) {
        ...
        plugin.getGUIManager().openGUI(new com.mteco.inventory.impl.nation.EnforcerManagementGUI(
    ...
    private void handleCrime(Player player) {
        ...
        plugin.getGUIManager().openGUI(new com.mteco.inventory.impl.nation.CrimeManagementGUI(
    ...
    private void handleLaws(Player player) {
        ...
        plugin.getGUIManager().openGUI(new com.mteco.inventory.impl.nation.LawsGUI(
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            ...
            if (sender.hasPermission("MTNations.admin")) completions.add("admin");
        ...
        if (args.length == 2 && args[0].equalsIgnoreCase("pcr")) {
            ...
            } else if (player.hasPermission("MTNations.admin")) {
        ...
        if (args.length == 2 && args[0].equalsIgnoreCase("admin")) {
            return filter(List.of("define"), args[1]);
        }
