...
public class MTCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 1 && args[0].equalsIgnoreCase("reload")) {
            if (!sender.hasPermission("mteco.reload")) {
        ...
        sender.sendMessage("§eUsage: §f/mteco reload §7| §f/mteco link §7| §f/mteco unlink");
        return true;
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            ...
            if (sender.hasPermission("mteco.reload")) completions.add("reload");
    ...
    private boolean handleDiscord(Player player, String[] args) {
        ...
        if (!plugin.getCharacterManager().hasCharacter(player.getUniqueId())) {
            player.sendMessage("§cYou need a character first. Use §e/mtc create§c.");
            return true;
        }
        ...
        if (args[0].equalsIgnoreCase("link")) {
            ...
            if (data.getDiscordId() != null) {
                player.sendMessage("§eYour character is already linked. Use §f/mteco unlink §eto remove it first.");
                return true;
            }
