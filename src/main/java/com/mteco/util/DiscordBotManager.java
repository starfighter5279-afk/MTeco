...
public class DiscordBotManager extends ListenerAdapter {
    ...
    public DiscordBotManager(MTeco plugin) {
        this.plugin = plugin;
        connect();
    }
    ...
    private void connect() {
        ...
        try {
            jda = JDABuilder.createDefault(token)
                    ...
                    .setActivity(Activity.playing("MTeco | /help"))
    ...
    public void onReady(ReadyEvent event) {
        ...
        postActivity("🤖 Bot Online", "MTeco Discord bot has connected.", 0x2ECC71);
    }
    ...
    private void registerSlashCommands(JDA api) {
        var commandList = java.util.List.of(
                ...
                Commands.slash("help", "View available MTeco Discord commands"),
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (playerUuid == null) {
            if (isDM && content.matches("\\d{6}")) {
                event.getChannel().sendMessage("❌ Invalid or expired code. Generate a new one with `/mteco link` in-game.").queue();
            }
            return;
        }
    ...
    public void onGuildVoiceUpdate(GuildVoiceUpdateEvent event) {
        if (plugin.getProximityManager() == null) return;
        ...
        if (lobbyId.equals(joinedId)) {
            event.getGuild().mute(event.getMember(), true).queue(null, e -> {});
        }

        boolean leftProximity = lobbyId.equals(leftId) || (leftId != null && plugin.getProximityManager().isManagedChannel(leftId));
        boolean joinedProximity = lobbyId.equals(joinedId) || (joinedId != null && plugin.getProximityManager().isManagedChannel(joinedId));
        if (leftProximity && !joinedProximity) {
            plugin.getProximityManager().removeTrackedPlayer(event.getMember().getId());
            event.getGuild().mute(event.getMember(), false).queue(null, e -> {});
        }
    }
    ...
    private void handleHelp(SlashCommandInteractionEvent event) {
        ...
        eb.setTitle("📋 MTeco Discord Commands");
        ...
        eb.setFooter("You must link your account with /mteco link in-game first.");
    ...
    private void handleWar(SlashCommandInteractionEvent event) {
        ...
        plugin.getServer().getScheduler().runTask(plugin, () -> {
            ...
            if (character == null) {
                event.getHook().sendMessage("❌ No linked character found. Use `/mteco link` in-game first.").queue();
                return;
            }
    ...
    private void handleFamilyInheritor(SlashCommandInteractionEvent event, CharacterData character) {
        ...
        if (character.getInheritorUuid() != null) {
            ...
            eb.setDescription("You have not set an inheritor. Use the in-game GUI (`/mtc` → Family → Manage Inheritor) to select one.");
        }
        ...
        eb.setFooter("Use /mtc → Family → Manage Inheritor in-game to change your inheritor.");
        event.getHook().sendMessageEmbeds(eb.build()).queue();
    }
    ...
    public void syncAllDiscordData() {
        ...
        if (plugin.getSettings().isDiscordMtcName()) {
