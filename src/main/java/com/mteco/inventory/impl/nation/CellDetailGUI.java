...
import com.mteco.data.CellData;
...
public class CellDetailGUI extends InventoryGUI {
    ...
    public CellDetailGUI(MTeco plugin, UUID nationId, UUID jailId, UUID cellId) {
        this.plugin = plugin;
            ...
            addButton(slot, new InventoryButton()
                    ...
                    .consumer(e -> {
                        ...
                        if (e.getClick() == ClickType.LEFT || e.getClick() == ClickType.SHIFT_LEFT) {
                            ...
                            if (r != null) {
                                r.setServing(false);
                                r.setJailReleaseTime(System.currentTimeMillis());
                                r.setCellId(null);
                                r.setAssignedBedLocation(null);
                                r.setEscaped(false);
                                plugin.getLawCrimeManager().saveCriminalRecord(r);
                                p.sendMessage("\u00a7aCriminal freed.");
                                Player criminal = Bukkit.getPlayer(r.getCriminalUUID());
                                if (criminal != null) criminal.sendMessage("\u00a7aYou have been released from jail.");
                            }
                            plugin.getGUIManager().openGUI(new CellDetailGUI(plugin, nationId, jailId, cellId), p);
                        } else {
