...
import com.mteco.util.BedrockUtil;
...
public class IDViewGUI extends InventoryGUI {
    ...
    public IDViewGUI(MTeco plugin, CharacterData targetData, UUID targetPlayerUUID, boolean isSelf) {
        this.plugin = plugin;
    ...
    public void decorate(Player viewer) {
        ...
        if (plugin.isMTBusinessEnabled() && targetData != null) {
            java.util.List<com.mteco.data.BusinessData> ownedBiz = plugin.getBusinessManager().getBusinessesByOwner(targetPlayerUUID);
            java.util.List<String> bizLore = new java.util.ArrayList<>();
            if (ownedBiz.isEmpty()) {
                bizLore.add("§7None");
            } else {
                for (com.mteco.data.BusinessData b : ownedBiz) bizLore.add("§f" + b.getName());
            }
            ...
            java.util.List<com.mteco.data.BusinessData> jobs = plugin.getBusinessManager().getBusinessesByEmployee(targetPlayerUUID);
            java.util.List<String> jobLore = new java.util.ArrayList<>();
            if (jobs.isEmpty()) {
                jobLore.add("§7None");
            } else {
                for (com.mteco.data.BusinessData b : jobs) {
