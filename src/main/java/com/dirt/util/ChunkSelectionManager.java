...
public class ChunkSelectionManager implements Listener {
    ...
    public ChunkSelectionManager(DirtEconomy plugin) {
        this.plugin = plugin;
    ...
    public void startManualClaim(Player player, UUID regionId, int radius) {
        ...
        player.sendMessage("\u00a7eLeft-click for the first corner, right-click for the second corner, then type \u00a76confirm\u00a7e or \u00a7ccancel\u00a7e.");
    }
    ...
    public void startAutoClaimForRegion(Player player, UUID regionId) {
        ...
        player.sendMessage("\u00a7eAuto-claim enabled for region \u00a76" + name + "\u00a7e. Walk over chunks to claim them. Use \u00a76/dr auto off\u00a7e to stop.");
    }
    ...
    public void onChat(AsyncPlayerChatEvent event) {
        ...
        if (msg.equals("confirm") && !session.awaitingConfirm) {
            player.sendMessage("\u00a7cSelect both corners first.");
            return;
        }
    ...
    private void processPropertySelection(Player player, UUID regionId, List<String> selectedChunks) {
        ...
        for (String ck : selectedChunks) {
            ...
            if (plugin.isDirtBusinessEnabled() && plugin.getBusinessManager() != null && plugin.getBusinessManager().getPropertyByChunk(ck) != null) { alreadyOwned++; continue; }
            validChunks.add(ck);
        }
        if (outsideRegion > 0) {
                    ...
                    plugin.getChatInputManager().requestInput(player,
                            ...
                            answer -> {
                                ...
                                if (region.getGovernorUUID() != null) {
                                    ...
                                    if (gov != null) gov.sendMessage("\u00a7eA new property purchase request is awaiting your approval. Check /dr.");
                                }
                                if (plugin.getRolesConfig().isTreasurerEnabled() && nation.getTreasurerUUID() != null) {
                                    ...
                                    if (tres != null) tres.sendMessage("\u00a7eA new property purchase request is awaiting your approval. Check /dr.");
                                }
                            }
                    );
                }
    ...
    private void processBusinessPropertySelection(Player player, UUID businessId, List<String> selectedChunks) {
        if (!plugin.isDirtBusinessEnabled()) return;
    ...
    private void processCellCreation(Player player, SelectionSession session) {
        ...
        com.dirt.data.CellData cell = plugin.getLawCrimeManager().createCell(
    ...
    private void processRoomCreation(Player player, SelectionSession session) {
        com.dirt.data.PropertyData prop = plugin.getNationManager().loadProperty(session.propertyId);
        if (prop == null) {
            com.dirt.data.BusinessPropertyData bprop = plugin.getBusinessManager() != null ? plugin.getBusinessManager().loadProperty(session.propertyId) : null;
            if (bprop == null) { player.sendMessage("\u00a7cProperty not found."); return; }
            com.dirt.data.RoomData room = plugin.getNationManager().createRoom(
    ...
    private void processExtendProperty(Player player, SelectionSession session, List<String> selectedChunks) {
        ...
        for (String ck : selectedChunks) {
            ...
            if (plugin.isDirtBusinessEnabled() && plugin.getBusinessManager() != null && plugin.getBusinessManager().getPropertyByChunk(ck) != null) { skipped++; continue; }
            validChunks.add(ck);
        }
        if (skipped > 0) {
