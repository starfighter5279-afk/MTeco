...
public class RoomManagementGUI extends InventoryGUI {
    ...
    public RoomManagementGUI(DirtEconomy plugin, UUID propertyId, boolean isBusiness, UUID contextId, int page, Consumer<Player> backCallback) {
        this.plugin = plugin;
    ...
    public void decorate(Player player) {
        ...
        List<UUID> roomIds;
        if (isBusiness) {
            com.dirt.data.BusinessPropertyData bprop = plugin.getBusinessManager().loadProperty(propertyId);
            roomIds = bprop != null ? bprop.getRoomIds() : new ArrayList<>();
        } else {
            com.dirt.data.PropertyData prop = plugin.getNationManager().loadProperty(propertyId);
