...
public class NationPayMembersGUI extends InventoryGUI {
    private final DirtEconomy plugin;
    private final UUID nationId;
    private final int page;

    public NationPayMembersGUI(DirtEconomy plugin, UUID nationId, int page) {
        this.plugin = plugin;
    ...
    protected Inventory createInventory() {
        return Bukkit.createInventory(null, 54, "\u00a7ePay Nation Member");
    }
    ...
    public void decorate(Player player) {
        ...
        for (int i = start; i < end; i++) {
            ...
            addButton(slot, new InventoryButton()
                    .creator(p -> {
                        if (c == null) return ItemUtil.buildItem(XMaterial.BARRIER, "\u00a7cUnknown Member");
                        return ItemUtil.buildItem(XMaterial.PLAYER_HEAD,
                                "\u00a7e" + c.getFirstName() + " " + c.getLastName(),
                                "\u00a77Click to pay this member.");
                    })
                    .consumer(e -> {
                        if (c == null) return;
                        ...
                        promptPayment(clicker, c.getFirstName() + " " + c.getLastName(), amount -> {
                            ...
                            if (n == null) return;
                            if (!plugin.getNationManager().withdrawFromTreasury(n.getName(), amount)) {
                                clicker.sendMessage("\u00a7cThe treasury does not have enough funds.");
                                plugin.getGUIManager().openGUI(new NationPayMembersGUI(plugin, nationId, page), clicker);
                                return;
                            }
                            plugin.getEconomy().depositPlayer(Bukkit.getOfflinePlayer(memberUUID), amount);
                            ...
                            if (target != null) target.sendMessage("\u00a7aYou received \u00a7e" + CurrencyUtil.symbol() + String.format("%.2f", amount) + "\u00a7a from the " + n.getName() + " treasury.");
                            plugin.getGUIManager().openGUI(new NationPayMembersGUI(plugin, nationId, page), clicker);
                        });
                    })
            );
        ...
        if (page > 0) {
            ...
            addButton(45, new InventoryButton()
                    ...
                    .consumer(e -> plugin.getGUIManager().openGUI(new NationPayMembersGUI(plugin, nationId, prev), (Player) e.getWhoClicked()))
            );
        }
        if (end < members.size()) {
            int next = page + 1;
            addButton(53, new InventoryButton()
                    ...
                    .consumer(e -> plugin.getGUIManager().openGUI(new NationPayMembersGUI(plugin, nationId, next), (Player) e.getWhoClicked()))
            );
        }
        ...
        plugin.getChatInputManager().requestInput(player,
                ...
                input -> {
                    ...
                    try { amount = Double.parseDouble(input); } catch (NumberFormatException ex) {
                        ...
                        plugin.getGUIManager().openGUI(new NationPayMembersGUI(plugin, nationId, page), player);
                        return;
                    }
                    if (amount <= 0) {
